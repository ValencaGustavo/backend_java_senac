...
public class DespertadorView {
    ...
    public static void main(String[] args) {
        ...
        while (sair == false) {
            DespertadorController.getHMS();
            tempoRestante = DespertadorController.tempoRestante();

            System.out.println(
                String.format(
                    "Agora são: %s%02d:%02d:%02d%s. O próximo alarme irá despertar às %s%02d:%02d%s. Tempo restante: %s%s%s. Adiado %s%d%s vez(es).", 
                    ANSI_YELLOW,
                    horaAtual, 
                    minutoAtual, 
                    segundoAtual,
